...
public class EntityVacca extends Entity
{
	...
	public EntityVacca(World worldIn)
	{
		super(worldIn);
	...
	public EntityVacca(World worldIn, double posX, double posY, double posZ)
	{
		this(worldIn);
	...
	{
		return (double)this.height * 0.0D - 0.2;
	}
		...
		else if (!this.worldObj.isRemote && !this.isDead)
		{
			...
			else
			{
				...
				if (flag || this.getDamageTaken() > 40.0F)
				{
					...
					if (!flag)
					{
						this.dropItemWithOffset(ItemVacca.instance, 1, 0.0F);
					}
			...
			if (!this.worldObj.isRemote)
			{
				...
				if (list != null && !list.isEmpty())
				{
					for (int k1 = 0; k1 < list.size(); ++k1)
					{
						...
						if (entity != this.riddenByEntity && entity.canBePushed() && entity instanceof EntityVacca)
						{
