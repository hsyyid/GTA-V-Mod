...
public class ItemZentorno extends Item
{
	//Following is so you can access it in pre-init
	public static final ItemZentorno instance = new ItemZentorno();
	public static final String name = "itemZentorno";
		...
		else
		{
			...
			else
			{
				if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
				{
					...
					EntityZentorno entityzentorno = new EntityZentorno(worldIn, (double)((float)blockpos.getX() + 0.5F), (double)((float)blockpos.getY() + 1.0F), (double)((float)blockpos.getZ() + 0.5F));
					entityzentorno.rotationYaw = (float)(((MathHelper.floor_double((double)(playerIn.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

					if (!worldIn.getCollidingBoundingBoxes(entityzentorno, entityzentorno.getEntityBoundingBox().expand(-0.1D, -0.1D, -0.1D)).isEmpty())
					{
						return itemStackIn;
					}
					...
					if (!worldIn.isRemote)
					{
						worldIn.spawnEntityInWorld(entityzentorno);
					}
