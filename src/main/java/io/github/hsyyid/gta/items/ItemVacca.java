package io.github.hsyyid.gta.items;
...
public class ItemVacca extends Item {
	//Following is so you can access it in pre-init
	public static final ItemVacca instance = new ItemVacca();
	public static final String name = "itemVacca";

	public ItemVacca()
	{
		this.maxStackSize = 1;
		setCreativeTab(CreativeTabs.tabTransport);
		setUnlocalizedName("gta:" + name.toLowerCase());
	}
		...
		else
		{
			...
			else
			{
				if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
				{
					...
					EntityVacca entityvacca = new EntityVacca(worldIn, (double)((float)blockpos.getX() + 0.5F), (double)((float)blockpos.getY() + 1.0F), (double)((float)blockpos.getZ() + 0.5F));
					entityvacca.rotationYaw = (float)(((MathHelper.floor_double((double)(playerIn.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

					if (!worldIn.getCollidingBoundingBoxes(entityvacca, entityvacca.getEntityBoundingBox().expand(-0.1D, -0.1D, -0.1D)).isEmpty())
					{
						List<Entity> e = worldIn.getEntitiesWithinAABBExcludingEntity(playerIn, entityvacca.getEntityBoundingBox());
					...
					if (!worldIn.isRemote)
					{
						worldIn.spawnEntityInWorld(entityvacca);
					}
