package com.arisux.airix.api.wavefrontapi;
...
import com.arisux.airix.client.render.Normal;
import com.arisux.airix.client.render.UV;
import com.arisux.airix.client.render.Vertex;
...
public class WavefrontModel
{
	...
	public boolean load(String modid, String path)
	{
		...
		{
			AIRIX.logger().warning("[WavefrontAPI] Error loading model for mod with id %s: %s", modid, path);
			e.printStackTrace();
			return false;
		}
		...
		{
			AIRIX.logger().warning("[WavefrontAPI] Error (%s) loading origins for model for mod with id %s: %s", e, modid, path);
		}
		...
		AIRIX.logger().info("[WavefrontAPI] Loaded wavefront model for mod with id %s: %s", this.modid, this.pathName);
	...
	private boolean loadOBJ(File fileModel, String path, Part part, FaceGroup fg) throws Exception
	{
		...
		{
			AIRIX.logger().bug("OBJ Loading Failed: " + path);
			stream.close();
			return false;
		}
	...
	private boolean loadMTL(File fileTexture, String path) throws Exception
	{
		...
		{
			AIRIX.logger().bug("MTL Loading failed: " + path);
			stream.close();
			return false;
		}
