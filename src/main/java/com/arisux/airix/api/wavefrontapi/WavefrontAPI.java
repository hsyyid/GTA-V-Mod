...
/**
 * AIRIX Wavefront Model Loader API
 * @author Windows
 */
public class WavefrontAPI
{
	...
	public WavefrontModel loadModel(Class<?> c, String modid, String model, String assetsPath)
	{
		...
		try
		{
			...
			if (!fileOBJ.exists())
			{
				...
				AIRIX.logger().info("Extracted wavefront model: %s", fileOBJ.getAbsoluteFile().getPath());
			}
			
			if (!fileMTL.exists())
			{
			    FileUtils.copyURLToFile(urlMTL, fileMTL);
			    AIRIX.logger().info("Extracted wavefront texture: %s", fileMTL.getAbsoluteFile().getPath());
			}
		}
		catch (Exception e)
		{
			AIRIX.logger().info("Error while extracting %s from %s: %s", path, assetsPath, e);
			e.printStackTrace();
		}
	...
	public WavefrontModel loadModel(String modid, File path)
	{
		...
		if (model.load(modid, path.getAbsolutePath()))
		{
			...
			AIRIX.logger().info("[WavefrontAPI] Loaded wavefront model: " + path);
		}
		else
		{
			AIRIX.logger().info("[WavefrontAPI] Unable to load wavefront model: " + path);
		}
